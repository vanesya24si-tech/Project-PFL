...
export default function Segmentation() {
  ...
  const totalDatabase = segments.reduce((acc, curr) => acc + curr.count, 0);
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="relative overflow-hidden rounded-[2rem] bg-white p-6 shadow-xl shadow-blue-900/5 border border-white flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          ...
          <div className="space-y-1">
            <h1 className="text-3xl font-black tracking-tight text-slate-800 italic uppercase flex items-center gap-3">
          ...
          <button className="inline-flex items-center justify-center gap-2 bg-gradient-to-r from-blue-600 to-sky-600 hover:brightness-105 text-white text-xs font-black px-5 py-3.5 rounded-2xl shadow-lg shadow-blue-500/10 transition-all cursor-pointer active:scale-95 shrink-0 uppercase tracking-widest">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
          ...
          <div className="lg:col-span-5 space-y-6">
            <div className="bg-white/80 backdrop-blur-md p-6 rounded-[2.5rem] border border-white shadow-xl shadow-blue-900/5">
              <h3 className="text-xs font-black text-slate-800 uppercase tracking-wider">Proporsi Distribusi Segmen</h3>
              <p className="text-xs text-slate-400 mt-0.5 font-medium">Persentase pembagian pangsa pasar operasional kasir.</p>
              ...
              <div className="h-56 w-full relative flex items-center justify-center mt-4">
                ...
                <div className="absolute text-center pointer-events-none">
                  <span className="text-3xl font-black text-slate-800 tracking-tight italic">{totalDatabase}</span>
              ...
              <div className="mt-4 space-y-2 pt-4 border-t border-slate-100">
                {segments.map((s) => (
                  <div key={s.name} className="flex items-center justify-between text-xs p-1 font-bold">
            ...
            <div className="bg-white/80 backdrop-blur-md border border-white p-5 rounded-[2rem] shadow-xl shadow-blue-900/5 flex items-center justify-between gap-4">
              <div className="space-y-1">
                ...
                <h4 className="text-lg font-black text-slate-800 tracking-tight">+5 Pelanggan Baru Minggu Ini</h4>
                <p className="text-xs text-slate-400 font-medium font-sans">Mengurangi tingkat churn rate hingga 12% bulan ini.</p>
              </div>
          ...
          <div className="lg:col-span-7 space-y-4">
            <div className="px-1">
              <h3 className="text-xs font-black text-slate-800 uppercase tracking-wider">Rekomendasi Kampanye & Eksekusi Otomatis</h3>
              <p className="text-xs text-slate-400 mt-0.5 font-medium">Kirim dorongan notifikasi WhatsApp / Voucher Blast berdasarkan target personal.</p>
            </div>
              ...
              {segments.map((s) => (
                ...
                >
                  <div className="flex items-start sm:items-center gap-4">
                    ...
                    <div 
                      className="w-11 h-11 shrink-0 rounded-2xl flex items-center justify-center text-lg text-white shadow-md transition-transform group-hover:scale-105" 
                    ...
                    <div className="space-y-1">
                      <div className="flex items-center gap-2 flex-wrap">
                        <h4 className="text-xs font-black text-slate-800 uppercase tracking-wide">{s.name}</h4>
                      ...
                      <p className="text-xs text-slate-500 font-medium leading-relaxed max-w-md">{s.desc}</p>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center justify-between sm:justify-end gap-6 pt-3 sm:pt-0 border-t border-slate-50 sm:border-0">
                    <div className="text-left sm:text-right sm:px-4 font-bold">
                      <span className="block text-lg font-black text-slate-800 leading-none">{s.count}</span>
