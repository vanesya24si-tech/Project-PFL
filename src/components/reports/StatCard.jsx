...
export default function StatCard({ icon, label, value, trend, trendColor }) {
  ...
  return (
    <div className="bg-white p-6 rounded-[2rem] shadow-sm border border-laundry-border flex items-center gap-5 relative overflow-hidden group hover:shadow-lg transition-all">
      <div className="p-4 rounded-2xl bg-laundry-100 text-aqua-bright text-3xl group-hover:scale-110 transition-transform">
        {icon}
      </div>
      <div>
        <p className="text-slate-500 text-sm font-bold uppercase tracking-wider">{label}</p>
        <h3 className="text-3xl font-black text-navy-deep mt-1">{value}</h3>
