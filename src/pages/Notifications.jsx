...
export default function Notifications() {
  ...
  const stats = [
    { label: "Template Aktif", value: templates.filter(t => t.isActive).length, icon: <HiMail className="text-2xl" />, color: "text-blue-600 bg-blue-50 border-blue-100" },
    { label: "Terkirim Hari Ini", value: 42, icon: <HiCheckCircle className="text-2xl" />, color: "text-emerald-600 bg-emerald-50 border-emerald-100" },
    { label: "Gagal Terkirim", value: 1, icon: <HiDocumentReport className="text-2xl" />, color: "text-rose-600 bg-rose-50 border-rose-100" },
    { label: "Total Riwayat", value: 128, icon: <HiClock className="text-2xl" />, color: "text-slate-600 bg-slate-50 border-slate-100" },
  ];
      ...
      <div className="max-w-6xl mx-auto space-y-8">
        ...
        <div className="flex flex-col gap-6 sm:flex-row sm:items-center sm:justify-between bg-white p-8 sm:p-10 rounded-[2.5rem] border border-slate-100 shadow-sm relative overflow-hidden">
          <div className="relative z-10">
            ...
            <h1 className="text-4xl sm:text-5xl font-black tracking-tight text-slate-900">Template <span className="text-blue-600">Notifikasi</span></h1>
            <p className="mt-2 text-base text-slate-500 max-w-md">Kelola pesan otomatis WhatsApp untuk menjaga hubungan dan loyalitas pelanggan Netto Laundry.</p>
          </div>
          <button onClick={() => alert("Fitur membuat template baru")} className="relative z-10 inline-flex items-center justify-center gap-2 rounded-2xl bg-blue-600 px-8 py-4 text-base font-bold text-white shadow-md shadow-blue-200 hover:bg-blue-700 active:scale-[0.95] transition-all">
            <HiPlus className="text-xl" /> Buat Template
          </button>
        </div>
        ...
        <section className="rounded-[2.5rem] bg-white border border-slate-100 shadow-xs overflow-hidden">
          <div className="border-b border-slate-100 p-8 flex items-center justify-between bg-linear-to-r from-white to-slate-50/50">
            <div>
              <h2 className="text-2xl font-black text-slate-900">Daftar Template Pesan</h2>
              <p className="text-sm text-slate-400 mt-1 uppercase font-bold tracking-widest">Klik kartu untuk pratinjau isi pesan</p>
            </div>
            ...
            {templates.map((template) => (
              ...
              >
                <div className="space-y-3 flex-1">
                  ...
                  <p className="text-base text-slate-500 max-w-2xl leading-relaxed font-medium">{template.description}</p>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-xs transition-all animate-fadeIn">
            <div className="bg-white rounded-[2.5rem] shadow-2xl border border-slate-100 w-full max-w-md overflow-hidden flex flex-col">
              ...
              <div className="p-6 bg-slate-50 border-b border-slate-100 flex items-center justify-between">
                <div>
                  <h4 className="font-black text-slate-900 text-xl">Pratinjau Template</h4>
                  <p className="text-sm text-slate-400 font-bold uppercase tracking-wider">{activePreview.title}</p>
                </div>
                <button 
              ...
              <div className="p-6 bg-slate-50 border-t border-slate-100 flex gap-3">
                <button 
                  ...
                  className="flex-1 py-3 px-4 rounded-xl border border-slate-200 bg-white font-bold text-sm text-slate-600 hover:bg-slate-100 transition-all"
                >
                  Ubah Teks
                </button>
                <button 
                  onClick={() => alert('Simulasi tes kirim sukses')}
                  className="flex-1 py-3 px-4 rounded-xl bg-emerald-600 font-black text-sm text-white hover:bg-emerald-700 shadow-md shadow-emerald-100 flex items-center justify-center gap-1.5 transition-all"
                >
                  <HiPaperAirplane className="rotate-90 text-base" /> Test Kirim WA
                </button>
              </div>
