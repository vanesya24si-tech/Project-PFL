...
export default function Member() {
  ...
  // 1. Batasi data yang muncul di tabel utama (Hanya 5 data teratas)
  const limitedCustomers = filteredCustomers.slice(0, 5);
  ...
  const segments = [
    { 
      name: "VIP", 
      count: customers.filter((c) => c.status?.toLowerCase() === "vip").length, 
      color: "#4f46e5", 
      icon: <HiUserGroup /> 
    },
    { 
      name: "Loyal", 
      count: customers.filter((c) => c.status?.toLowerCase() === "loyal").length, 
      color: "#0284c7", 
      icon: <HiStar /> 
    },
    { 
      name: "Reguler", 
      count: customers.filter((c) => c.status?.toLowerCase() === "reguler").length, 
      color: "#38bdf8", 
      icon: <HiTrendingUp /> 
    },
    { 
      name: "Baru", 
      count: customers.filter((c) => c.status?.toLowerCase() === "baru").length, 
      color: "#7dd3fc", 
      icon: <HiSparkles /> 
    },
  ];
          ...
          <section className="rounded-[2rem] bg-white border border-slate-100 p-6 shadow-xs flex flex-col justify-between">
            ...
            {filteredCustomers.length > 5 && (
              <div className="mt-5 flex justify-center border-t border-slate-100 pt-4">
                ...
                >
                  Lihat Selengkapnya ({filteredCustomers.length} Pelanggan)
