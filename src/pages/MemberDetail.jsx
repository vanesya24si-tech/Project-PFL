  ...
  useEffect(() => {
    async function fetchCustomer() {
      try {
        ...
        console.error("Gagal memuat pelanggan di MemberDetail:", err);
      } finally {
        setLoading(false);
      }
    }
  ...
  const handleDelete = async () => {
    if (window.confirm(`Apakah Anda yakin ingin menghapus data ${customer?.name}?`)) {
      try {
        ...
        console.error("Gagal menghapus pelanggan dari Supabase:", err);
    ...
    return (
      <div className="min-h-screen bg-[#FDFDFD] p-6 flex items-center justify-center font-sans">
        <div className="text-center">
          <span className="inline-block animate-spin rounded-full h-8 w-8 border-b-2 border-blue-600 mb-2"></span>
          <p className="text-base text-slate-500 font-bold">Memuat profil pelanggan...</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen bg-[#FDFDFD] p-6 flex items-center justify-center font-sans">
        <div className="max-w-md rounded-3xl bg-white p-10 shadow-sm border border-slate-100 text-center">
          <p className="text-sm uppercase tracking-[0.2em] text-slate-400 font-semibold mb-3">Notifikasi</p>
          <h2 className="text-3xl font-bold text-slate-800 mb-3">Profil Tidak Ditemukan</h2>
          <p className="text-base text-slate-500 mb-8 leading-relaxed">Maaf, data pelanggan yang Anda cari tidak ada atau sudah dihapus.</p>
          <Link to="/members" className="inline-flex items-center justify-center rounded-full bg-sky-600 px-7 py-3 text-base font-semibold text-white hover:bg-sky-700 transition duration-150 shadow-sm">
            Kembali ke Daftar
          </Link>
        </div>
      </div>
    ...
    <div className="min-h-screen bg-[#FDFDFD] p-6 md:p-10 font-sans text-slate-700">
      <div className="max-w-7xl mx-auto space-y-10">
        ...
        <div className="flex flex-col gap-6 sm:flex-row sm:items-center sm:justify-between border-b border-slate-100 pb-8">
          <div>
            <div className="flex items-center gap-2 text-base text-slate-400 mb-2">
              <Link to="/members" className="hover:text-sky-600 transition">Pelanggan</Link>
              <span>/</span>
              <span className="text-slate-500 font-medium">{customer.name}</span>
            </div>
            <h1 className="text-4xl font-extrabold text-slate-900 tracking-tight">Detail Profil</h1>
          </div>
          <div className="flex flex-wrap items-center gap-3">
            <button onClick={() => navigate(-1)} className="inline-flex items-center gap-2 rounded-full border border-slate-200 bg-white px-5 py-2.5 text-base font-semibold text-slate-600 hover:bg-slate-50 transition shadow-sm hover:border-slate-300">
              <HiChevronLeft className="text-xl text-slate-400" /> Kembali
            </button>
            <Link to={`/members/${id}/edit`} className="inline-flex items-center gap-2 rounded-full bg-white border border-slate-200 px-5 py-2.5 text-base font-semibold text-sky-700 hover:bg-sky-50 transition shadow-sm hover:border-sky-200">
              <HiOutlinePencil className="text-xl" /> Edit
            </Link>
            <button onClick={handleDelete} className="inline-flex items-center gap-2 rounded-full bg-red-50 px-5 py-2.5 text-base font-semibold text-red-700 hover:bg-red-100 transition shadow-sm">
        ...
        <div className="grid gap-8 lg:grid-cols-[1fr_minmax(340px,auto)]">
          ...
          <div className="space-y-8">
            <div className="rounded-3xl bg-white p-8 shadow-sm border border-slate-100">
              <div className="flex flex-col gap-6 md:flex-row md:items-center">
                ...
                <div className="flex h-24 w-24 flex-shrink-0 items-center justify-center rounded-3xl bg-sky-100 text-5xl font-bold text-sky-700 shadow-inner">
                ...
                <div className="flex-grow">
                  ...
                  <h2 className="text-4xl font-extrabold text-slate-900 tracking-tight">{customer.name}</h2>
              ...
              <div className="grid gap-5 mt-10 pt-8 border-t border-slate-100 sm:grid-cols-2">
                ...
                <div className="flex items-start gap-4 p-2 sm:col-span-2">
                  ...
                  <div>
                    ...
                    <p className="mt-1 text-lg font-semibold text-slate-800 leading-relaxed">{customer.address || "Alamat belum diatur"}</p>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className="space-y-6">
            <h3 className="text-xl font-bold text-slate-800 px-1">Ikhtisar Akun</h3>
            <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-1">
              ...
              <div className="rounded-2xl bg-white p-6 shadow-sm border border-slate-100 flex items-center gap-5">
                ...
                <div>
                  ...
                  <p className="mt-1 text-4xl font-bold text-slate-900">{totalTransactions}<span className="text-xl font-medium text-slate-400 ml-1.5">kali</span></p>
                </div>
              </div>
              ...
              <div className="rounded-2xl bg-white p-6 shadow-sm border border-slate-100 flex items-center gap-5">
                ...
                <div>
                  ...
                  <p className="mt-1 text-3xl font-bold text-slate-900">Rp {totalSpent.toLocaleString("id-ID")}</p>
                </div>
              </div>
              ...
              <div className="rounded-2xl bg-sky-700 p-7 shadow-md text-white">
                <div className="flex items-center justify-between gap-4 mb-5">
                  ...
                  <HiSparkles className="text-3xl text-sky-300" />
                </div>
                <p className="text-5xl font-extrabold">{points} <span className="text-2xl font-medium text-sky-200">pts</span></p>
                <div className="mt-4 pt-4 border-t border-sky-600/50 text-base font-medium text-sky-100">
        ...
        <div className="rounded-3xl bg-white p-8 md:p-10 shadow-sm border border-slate-100">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between mb-9 pb-6 border-b border-slate-100">
            <div>
              <h2 className="text-3xl font-extrabold text-slate-900 tracking-tight">Riwayat Transaksi</h2>
              <p className="mt-1 text-base text-slate-500">Menampilkan {transactions.length} aktivitas terbaru</p>
            </div>
            <div className="text-base font-semibold text-slate-500 bg-slate-50 px-5 py-2.5 rounded-full border border-slate-100">
                ...
                {transactions.map((trx) => (
                  <tr key={trx.id} className="hover:bg-slate-50/50 transition-colors">
                    ...
                    <td className="px-5 py-5">
                      ...
                      <span className="text-sm text-slate-400 font-medium">{trx.details} • {trx.payment}</span>
                    </td>
                    <td className="px-5 py-5 font-bold text-sky-700">Rp {trx.total.toLocaleString("id-ID")}</td>
                    <td className="px-5 py-5">
                      <span className={`inline-flex rounded-full px-3.5 py-1 text-sm font-semibold ${
          ...
          {transactions.length === 0 && (
            <div className="text-center py-12 text-slate-400 text-base border border-dashed border-slate-200 rounded-2xl mt-4">
