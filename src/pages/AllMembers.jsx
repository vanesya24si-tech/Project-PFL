  ...
  useEffect(() => {
    ...
    async function fetchAll() {
      try {
        ...
        console.error("Gagal memuat pelanggan di AllMembers:", err);
      } finally {
        if (active) {
          setLoading(false);
        }
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="flex items-center justify-between">
          <Link 
            ...
            className="inline-flex items-center gap-2 rounded-2xl border border-blue-100 bg-white px-4 py-2.5 text-sm font-black uppercase tracking-wider text-slate-600 hover:bg-blue-50 transition-all active:scale-95 shadow-xs"
          >
            <HiArrowLeft className="text-lg" /> Kembali ke Dashboard
          </Link>
          <div className="text-right">
            <p className="text-sm font-bold text-slate-400">Total Database</p>
            <p className="text-xl font-black text-blue-600">{customers.length} Pelanggan</p>
          </div>
        </div>
        ...
        <section className="rounded-[2rem] bg-white border border-slate-100 p-8 shadow-lg shadow-blue-100/30">
          ...
          <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between border-b border-slate-100 pb-6">
            <div className="flex items-center gap-3">
              <div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-blue-50 text-blue-600 text-3xl border border-blue-100">
                <HiUsers />
              </div>
              <div>
                <h1 className="text-3xl font-black text-slate-900 tracking-tight">Basis Data Seluruh Pelanggan</h1>
                <p className="text-sm text-slate-400 font-medium">Melihat dan mengelola seluruh riwayat data CRM tanpa batas.</p>
              </div>
            </div>
          </div>
                  ...
                  {loading ? (
                    <tr>
                      <td colSpan="5" className="px-6 py-12 text-center text-slate-400 font-medium">
                        ...
                        Memuat basis data pelanggan dari database...
                      </td>
                    </tr>
                  ) : filteredCustomers.length === 0 ? (
                    <tr>
                      <td colSpan="5" className="px-6 py-12 text-center text-slate-400 font-medium">
                        <HiOutlineUserCircle className="text-5xl mx-auto mb-2 text-slate-300" />
                    ...
                    filteredCustomers.map((customer) => {
                      ...
                      return (
                        <tr key={customer.id} className="hover:bg-blue-50/20 transition-colors group">
                          ...
                          {/* Total Order */}
                          <td className="px-6 py-4 text-center text-slate-700 font-mono text-base">
                            {customer.orders || 0}
                          </td>
                          {/* Aksi */}
                          <td className="px-6 py-4 text-right">
                            <Link to={`/members/${customer.id}`} className="inline-flex items-center gap-0.5 text-blue-600 hover:text-blue-700">
                              Profil Lengkap <HiChevronRight className="text-base" />
                            </Link>
                          </td>
                        </tr>
                      );
          ...
          <div className="mt-4 flex items-center justify-between text-sm text-slate-400 font-medium px-2">
