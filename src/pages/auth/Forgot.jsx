...
export default function Forgot() {
  ...
  const handleResetSubmit = async (e) => {
    ...
    try {
      ...
      setIsSent(true);
      setSuccess("Tautan pemulihan kata sandi berhasil dikirim! Silakan periksa email Anda.");
    } catch (err) {
      console.error("Recovery error:", err);
      let errMsg = "Gagal mengirim email pemulihan.";
      if (err) {
      ...
      if (errMsg === "{}" || errMsg === "null" || !errMsg) {
        errMsg = "Terjadi kesalahan koneksi atau konfigurasi Supabase. Pastikan email terdaftar sudah benar.";
      }
      setError(errMsg);
    } finally {
      setIsLoading(false);
    ...
    <div className="min-h-screen w-full bg-gradient-to-br from-[#F0F7FF] via-[#F8FAFC] to-[#E0F2FE] flex items-center justify-center p-4 md:p-8 text-[#0F172A] antialiased font-sans relative overflow-hidden">
      ...
      <div className="w-full max-w-md bg-white/80 backdrop-blur-xl rounded-[2.5rem] border border-white/60 shadow-[0_32px_64px_-16px_rgba(2,132,199,0.1)] p-6 md:p-8 relative z-10 transition-all animate-in fade-in slide-in-from-bottom-4 duration-500">
        ...
        <div className="text-center">
          ...
          <h2 className="text-4xl font-black text-slate-800 leading-tight tracking-tight uppercase italic">
            LUPA KATA <span className="text-blue-500 font-black not-italic">SANDI?</span>
          </h2>
          <p className="mt-2 text-sm font-medium text-slate-400 max-w-xs mx-auto leading-relaxed">
        ...
        {/* ALERT MESSAGES */}
        {error && (
          <div className="mb-4 p-4 text-sm font-bold text-rose-600 bg-rose-50 rounded-2xl border border-rose-100 animate-pulse">
            {error}
          </div>
        )}
        {success && (
          <div className="mb-4 p-4 text-sm font-bold text-emerald-600 bg-emerald-50 rounded-2xl border border-emerald-100">
            {success}
          </div>
        )}
        ...
        <form className="space-y-5" onSubmit={handleResetSubmit}>
          ...
          <div className="space-y-1.5">
            ...
            <div className="relative group">
              ...
              <input
                ...
                className="w-full rounded-2xl border border-slate-200/80 bg-slate-50/40 pl-11 pr-4 py-3.5 text-sm font-bold text-slate-800 outline-none transition-all focus:border-blue-500 focus:bg-white focus:ring-4 focus:ring-blue-500/5 shadow-inner disabled:opacity-60"
              />
            </div>
          </div>
          ...
          <button
            type="submit"
            disabled={isLoading || isSent}
            className="w-full inline-flex items-center justify-center gap-2 rounded-2xl bg-gradient-to-r from-blue-600 to-sky-600 px-6 py-4 text-sm font-black uppercase tracking-widest text-white shadow-lg shadow-blue-500/20 transition-all hover:shadow-xl hover:shadow-blue-500/30 hover:brightness-105 active:scale-[0.98] disabled:from-slate-400 disabled:to-slate-500 cursor-pointer"
          >
            <HiPaperAirplane className={`text-base rotate-45 transition-transform ${isLoading || isSent ? '' : 'group-hover:translate-x-0.5 group-hover:-translate-y-0.5'}`} />
            {isLoading ? "Mengirim..." : isSent ? "Tautan Terkirim" : "Kirim Instruksi"}
          </button>
        </form>
        ...
        <div className="mt-8 pt-4 border-t border-slate-100">
          <Link
            to="/login"
            className="flex items-center justify-center gap-2 text-sm font-bold text-slate-400 hover:text-blue-600 tracking-tight transition-colors group"
          >
