...
export default function AddOrderForm() {
  ...
  // Database pelanggan existing (Jika storage kosong, pakai mock data default sesuai mockup Anda)
  const [customers] = useState(() => {
    const saved = loadCustomers();
    return saved && saved.length > 0 ? saved : [
      { id: "MBR-001", name: "Andi Saputra", phone: "081234567890", email: "andi@gmail.com", status: "Silver", customerType: "Pekerja/Karyawan" },
      { id: "MBR-002", name: "Siti Aisyah", phone: "085298765432", email: "siti@gmail.com", status: "Platinum", customerType: "Ibu Rumah Tangga" },
      { id: "MBR-003", name: "Budi Santoso", phone: "082177665544", email: "budi@gmail.com", status: "Gold", customerType: "Pekerja/Karyawan" },
      { id: "MBR-004", name: "Dewi Lestari", phone: "081399887766", email: "dewi@gmail.com", status: "Silver", customerType: "Pelajar/Mahasiswa" }
    ];
  });
  ...
  const [selectedProductId, setSelectedProductId] = useState(() => loadProducts()[0]?.id || "");
      ...
      <div className="max-w-3xl mx-auto space-y-6">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="bg-white p-6 rounded-[2rem] border border-slate-100 shadow-xs space-y-5">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-50 pb-3">
              ...
              {!isExistingCustomer && (
                <button
                  ...
                  className="inline-flex items-center gap-2 rounded-xl bg-blue-50 border border-blue-100 px-4 py-2 text-xs font-black uppercase text-blue-600 hover:bg-blue-100 transition-all cursor-pointer"
                >
            ...
            {isExistingCustomer ? (
                ...
                <div className="flex items-center gap-3.5">
                  ...
                  <div>
                    <div className="flex items-center gap-2">
                      <p className="font-black text-sm text-slate-800">{crmData.customerName}</p>
                      <span className="text-[9px] font-black uppercase px-2 py-0.5 rounded-md bg-slate-900 text-white tracking-wider font-mono">
                        {crmData.segment}
                      </span>
                    </div>
                    <p className="text-xs text-slate-500 font-bold mt-0.5">ID: {crmData.id} • Telp: {crmData.phone}</p>
                  </div>
                </div>
                <button
                  type="button"
                  onClick={handleResetCustomer}
                  className="text-xs font-black text-red-500 hover:underline tracking-tight uppercase"
                >
                  Ganti Pelanggan
                </button>
              </div>
              ...
              <div className="space-y-4">
                ...
                {isRegisteringMember && (
                  <div className="p-4 bg-blue-50/20 border border-dashed border-blue-100 rounded-2xl grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs font-bold animate-fadeIn">
                    <div className="space-y-1.5">
          ...
          <div className="bg-white p-6 rounded-[2rem] border border-slate-100 shadow-xs space-y-4">
            <h3 className="text-xs font-black text-slate-800 uppercase tracking-wider">Layanan & Timbangan (Kiloan)</h3>
              ...
              <div className="p-4 bg-emerald-50/40 border border-emerald-100 rounded-2xl space-y-2">
                ...
                <div className="relative max-w-xs">
                  ...
                  <input 
                    ...
                    className="w-full pl-10 pr-4 py-2.5 text-xs font-black bg-white border border-emerald-200 rounded-xl text-slate-800 focus:outline-none" 
                  />
                </div>
              </div>
            )}
          ...
          <button
            ...
            className="w-full inline-flex items-center justify-center gap-2 bg-blue-600 hover:bg-blue-700 disabled:opacity-60 disabled:cursor-not-allowed text-white text-xs font-black px-5 py-4 rounded-2xl shadow-lg transition-all cursor-pointer uppercase tracking-wider active:scale-95"
          >
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-xs animate-fadeIn">
          <div className="bg-white w-full max-w-md rounded-[2.5rem] border border-slate-100 shadow-2xl p-6 space-y-5 relative flex flex-col max-h-[85vh]">
            ...
            <div className="text-left">
              <h3 className="text-base font-black text-slate-800 tracking-tight uppercase">Pilih Pelanggan</h3>
            ...
            <div className="relative">
              <HiSearch className="absolute left-4 top-1/2 -translate-y-1/2 text-blue-500 text-base" />
              <input 
                type="text"
                placeholder="Masukkan nama member atau nomor HP..."
                value={searchMemberQuery}
                onChange={(e) => setSearchMemberQuery(e.target.value)}
                className="w-full pl-11 pr-4 py-3 text-xs font-bold text-slate-800 bg-white border-2 border-blue-500 rounded-2xl outline-none focus:ring-4 focus:ring-blue-100 transition-all"
              />
            </div>
            ...
            <div className="text-left space-y-1.5 overflow-y-auto flex-1 pr-1">
              ...
              {filteredMembers.length === 0 ? (
                <p className="text-xs font-bold text-slate-400 py-6 text-center">Member tidak ditemukan dalam database.</p>
              ) : (
                filteredMembers.map((customer) => (
                  ...
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="text-xs font-black text-slate-800 group-hover:text-blue-600 transition-colors">{customer.name}</p>
            ...
            <button
              ...
              className="w-full bg-[#111827] text-white py-3 rounded-2xl text-xs font-black uppercase tracking-wider inline-flex items-center justify-center gap-2 transition-all active:scale-98"
            >
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/70 backdrop-blur-xs animate-fadeIn">
          <div className="bg-white w-full max-w-md rounded-[2.5rem] p-6 text-center space-y-4 relative overflow-y-auto max-h-[90vh] shadow-2xl">
            ...
            <div className="space-y-1 pt-2">
              ...
              <h3 className="text-xl font-black text-slate-800 mt-2">IDR {totalPrice.toLocaleString("id-ID")}</h3>
              <p className="text-[11px] font-bold text-slate-400">Nota: {generatedOrderId} • {crmData.customerName} ({weight} KG)</p>
            </div>

            {paymentMethod === "cash" && (
              <div className="p-3 bg-emerald-50/50 border border-emerald-100 rounded-xl text-left grid grid-cols-2 gap-y-1 text-xs font-bold">
            ...
            <div className="grid grid-cols-2 gap-3 pt-2">
              <button type="button" onClick={() => alert("Cetak struk sukses.")} className="border bg-white text-slate-700 text-xs font-black py-3 rounded-2xl uppercase"><HiPrinter size={16} /> Cetak Struk</button>
              <button type="button" onClick={() => { setShowInvoiceModal(false); navigate("/orders"); }} className="bg-slate-900 text-white text-xs font-black py-3 rounded-2xl uppercase">Selesai <HiArrowSmRight size={16} /></button>
            </div>
          </div>
        </div>
      )}
